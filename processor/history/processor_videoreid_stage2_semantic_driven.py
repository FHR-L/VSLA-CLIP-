    ...
    with torch.no_grad():
        ...
        for i in range(i_ter):
            ...
            with amp.autocast(enabled=True):
                text_feature = model(label=l_list, get_text_only_shared=True)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam) in enumerate(train_loader_stage2):
            ...
            if cfg.MODEL.PBP_VIEW:
                ...
                target_view = None
            with amp.autocast(enabled=True):
                score, feat, image_features, concat_feat = model(
                    x=img,
                    label=target,
                    cam_label=cam_label,
                    view_label=target_view
                )
                text_feature = model(label=target, get_text_only_shared=True)
                ...
                # print(feat.shape)
                # assert 1 < 0
                loss = loss_fn(score, feat, target, cam_label, logits, loss_t2i, loss_i2t, concat_feat)
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                ...
                # # A2G
                # cmc, mAP, ranks = test(model, query_loader_a2g, gallery_loader_a2g, use_gpu, cfg)
                # ptr = "A2G mAP: {:.2%}".format(mAP)
                # for r in ranks:
                #     ptr += " | R-{:<3}: {:.2%}".format(r, cmc[r - 1])
                # logger.info(ptr)
