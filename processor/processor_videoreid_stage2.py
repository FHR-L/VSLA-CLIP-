    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam) in enumerate(train_loader_stage2):
            ...
            with amp.autocast(enabled=True):
                ...
                part_target = torch.tensor([i + 1 for i in range(4)]).to(device)
                            ...
                            for n in range(4):
                                if cam_id[m] == cam_id[n]:
                                    ...
                                    if cross_platform_align_loss is None:
                                        cross_platform_align_loss = (xent_query(feat1, feat2, part_target, part_target) + xent_query(feat2, feat1, part_target, part_target))
                                        loss_count += 1.
                                    else:
                                        cross_platform_align_loss += (xent_query(feat1, feat2, part_target, part_target) + xent_query(feat2, feat1, part_target, part_target))
                                        loss_count += 1.
...
def visualize(cfg,
    ...
    model.eval()
    for n_iter, (img, vid, target_cam, img_path) in enumerate(val_loader):
        # print(model.prompt_learner.text_shared_prompts[0])
        # TODO
        target_view = target_cam
        img = img.to(device)
