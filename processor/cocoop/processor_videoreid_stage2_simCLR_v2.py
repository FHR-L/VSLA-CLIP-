    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam) in enumerate(train_loader_stage2):
            ...
            with amp.autocast(enabled=True):
                ...
                text_features_new = model.text_transform(text_features)
                text_features_new = torch.squeeze(text_features_new, dim=1)
                text_features_old = torch.squeeze(text_features, dim=1)
                # logits_old = image_features @ text_features_old.t()
                logits = image_features @ (text_features_new + text_features_old).t()
