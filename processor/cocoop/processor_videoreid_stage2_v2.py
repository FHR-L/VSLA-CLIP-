...
def do_train_stage2(cfg,
    ...
    image_features = []
    labels = []
    image_features_order_by_id = []
    with torch.no_grad():
        for n_iter, (vids, pids, camid) in enumerate(train_loader_stage2):
            print(n_iter)
            vids = vids.to(device)
            target = pids.to(device)
            with amp.autocast(enabled=True):
                _, image_feature = model(vids, target, get_image=True)
                for i, img_feat in zip(target, image_feature):
                    labels.append(i)
                    image_features.append(img_feat)
        for i in range(max(labels) + 1):
            image_features_order_by_id.append([])

        for id, img_feat in zip(labels, image_features):
            image_features_order_by_id[id].append(img_feat)
        image_features_order_by_id_list = []
        for feat_list in image_features_order_by_id:
            image_features_order_by_id_list.append(torch.mean(torch.stack(feat_list, dim=0), dim=0))
        # labels_list = torch.stack(labels, dim=0).cuda()  # N
        # image_features_list = torch.stack(image_features, dim=0).cuda()
        image_features_order_by_id_list = torch.stack(image_features_order_by_id_list, dim=0).cuda()
    ...
    with torch.no_grad():
        for i in range(i_ter):
            ...
            img_feat = image_features_order_by_id_list[l_list]
            with amp.autocast(enabled=True):
                text_feature = model(label=l_list, get_text=True, image_feature_to_get_text=img_feat)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam) in enumerate(train_loader_stage2):
            ...
            with amp.autocast(enabled=True):
                ...
                text_feature = model(label=target, get_text=True, image_feature_to_get_text=feat[1])
                loss_i2t = xent(image_features, text_feature, target, target)
                loss_t2i = xent(text_feature, image_features, target, target)
                logits = image_features @ text_features.t()
                loss = loss_fn(score, feat, target, target_cam, logits) + loss_i2t + loss_t2i
