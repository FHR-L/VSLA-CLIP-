...
class PartDecoder(nn.Module):
    def __init__(self,  num_layers, num_query, pos_embed, frame_num=8, use_cross_frame_attn=False):
        super().__init__()
    ...
    def forward(self, x = None, label=None, get_image = False, get_text = False, cam_label= None, view_label=None):
        ...
        if self.use_part_model:
            ...
            if self.use_temporal_weight:
                part_features_query, attn_map = self.part_decoder.forward_cls_as_query_temporal_weight(image_features[:, 1:],
                                                                             image_features[:, 0])
            ...
            part_features_split = self.part_decoder.forward_part(image_features[:, 1:], image_features[:, 0])
            part_features_split = einops.rearrange(part_features_split, '(b t) l d -> b t l d', b=batch)
            part_features_split = torch.mean(part_features_split, dim=1)
        else:
        ...
        if self.training:
            ...
            if self.use_part_model:
                # [img_feature,  part_feature_concat]
                # [part_feature_concat]
                # [img_feature_proj, part_feature_concat]
