...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, attn_mask: torch.Tensor=None):
        if attn_mask is None:
            ...
            attn_mask = attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
...
class TEMPORAL_BLOCK_V2(nn.Module):
    ...
    def forward(self, x):
        ...
        cls = einops.rearrange(cls, 'l (b t) d -> t (b l) d', t=self.T)
        cls = self.do_temporal_attn(cls)
        cls = einops.rearrange(cls, 't (b l) d -> l (b t) d', l=1)
        x = torch.cat([cls, patches], dim=0)
        # print(x.shape)
        # assert 1 < 0
        return x

class TEMPORAL_BLOCK_V3(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, droppath=0., T=0, ):
        super().__init__()
        self.T = T
        self.temporal_attn = nn.MultiheadAttention(d_model, n_head)
    ...
    def forward(self, x, attn_map):
        l, bt, d = x.size()
        # mask
        _, topk_indices = torch.topk(attn_map[:, 0, 1:], int(l-1 * self.ratio), dim=1)
        topk_indices = torch.sort(topk_indices, dim=1).values
        selected_tokens_mask = torch.zeros((bt, l-1), dtype=torch.bool).cuda()
        ...
        cls = einops.rearrange(cls, 't (b l) d -> l (b t) d', l=1)
        #
        mask_patches = einops.rearrange(patches, 'l (b t) d -> (b t) l d', t=self.T)
        # print(selected_tokens_mask.shape)
        mask_patches = mask_patches.to(dtype=x.dtype) * selected_tokens_mask.unsqueeze(-1)
        # assert 1 <  0
        mask_patches = einops.rearrange(mask_patches, '(b t) l d -> t (b l) d', t=self.T)
        mask_patches = self.do_temporal_attn(mask_patches)
        mask_patches = einops.rearrange(mask_patches, 't (b l) d -> l (b t) d', l=l-1)
        x = torch.cat([cls, mask_patches], dim=0)
        # print(x.shape)
        # assert 1 < 0
        return x

# Video Set-Level-Adapter
class ResidualAttentionBlock_DAT_CROSS_ATTENTION_V3(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None, alpha=1, byta=1):
        super().__init__()
        ...
        print(f"alpha:{self.alph}, byta:{self.byta}")

        self.dat = nn.Sequential(OrderedDict([  # intra-frame attention adapter
            ("down", nn.Linear(d_model, d_model // self.alph)),
            ("gelu", QuickGELU()),
            ("up", nn.Linear(d_model // self.alph, d_model))
        ]))
        # self.dat_cross = nn.Sequential(OrderedDict([  # inter-frame cross-attention adapter
        #     ("down", nn.Linear(d_model, d_model // self.byta)),
        #     ("temporal_block", TEMPORAL_BLOCK_V3(d_model // self.byta, d_model // self.byta // 64, attn_mask, T=seq_len)),
        #     ("up", nn.Linear(d_model // self.byta, d_model))
        # ]))
        self.dat_cross_down = nn.Linear(d_model, d_model // self.byta)
        self.dat_cross_temporal_block = TEMPORAL_BLOCK_V3(d_model // self.byta, d_model // self.byta // 64, attn_mask, T=seq_len)
        self.dat_cross_up = nn.Linear(d_model // self.byta, d_model)
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
        self.deep = index
        ...
        if use_dat:
            if cv_emb != None and self.deep < cv_emb.shape[0]:  # [deep l (b t) d]
                ...
                if self.deep < 0:
                    ...
                    x1, attn_map = self.attention_weight(self.ln_1(x))
                    x2 = self.dat_cross_down(x)
                    x2 = self.dat_cross_temporal_block(x2, attn_map)
                    x2 = self.dat_cross_up(x2)
                    x = x + x1 + x2
...
# Video Set-Level-Adapter
class ResidualAttentionBlock_DAT_CROSS_ATTENTION_V2(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None, alpha=1, byta=1):
        super().__init__()
        ...
        self.dat_cross = nn.Sequential(OrderedDict([  # inter-frame cross-attention adapter
            ("down", nn.Linear(d_model, d_model // self.byta)),
            ("temporal_block", TEMPORAL_BLOCK_V2(d_model // self.byta, d_model // self.byta // 64, attn_mask, T=seq_len)),
            ("up", nn.Linear(d_model // self.byta, d_model))
        ]))
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
...
# Video Set-Level-Adapter
class ResidualAttentionBlock_DAT_CROSS_ATTENTION_V1_Norm(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None, alpha=1, byta=1):
        super().__init__()
        ...
        self.dat_cross = nn.Sequential(OrderedDict([  # inter-frame cross-attention adapter
            ("down", nn.Linear(d_model, d_model // self.byta)),
            ("temporal_block", TEMPORAL_BLOCK(d_model // self.byta, d_model // self.byta // 64, attn_mask, T=seq_len)),
            ("up", nn.Linear(d_model // self.byta, d_model))
        ]))
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
    ...
    def forward_(self, input):
        x, use_dat, cv_emb, semantic_prompt = input

        if use_dat:
            if cv_emb is not None:
                assert 0
            if semantic_prompt is not None:
                l = semantic_prompt.shape[0]
                # x_n = self.ln_1(x)
                x_pbp = torch.cat([x, semantic_prompt], dim=0)
                x_pbp = self.ln_1(x_pbp)
                x_pbp_out = self.attention(x_pbp)
                x_pbp_out = x_pbp_out[:-l]
                x = x + x_pbp_out + self.dat_cross(x)
            else:
    ...
    def forward_(self, input):
        ...
        if use_dat:
            ...
            if semantic_prompt is not None:
                l = semantic_prompt.shape[0]
                x_n = self.ln_1(x)
                x_pbp = torch.cat([x_n, semantic_prompt], dim=0)
                x_pbp_out = self.attention(x_pbp)
                x_pbp_out = x_pbp_out[:-l]
                x = x + x_pbp_out + self.dat_cross(x)
            else:
...
class ResidualAttentionBlock_Base(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None, alpha=1, byta=1):
        super().__init__()
...
# Video Set-Level-Adapter
class ResidualAttentionBlock_DAT_CROSS_ATTENTION_IMAGE(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None, alpha=1, byta=1):
        super().__init__()
...
class Transformer_DAT_PT(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, seq_len: int, attn_mask: torch.Tensor = None, dat_type='v1', alpha=1, byta=1):
        super().__init__()
        self.width = width
        self.layers = layers
        if dat_type == 'v1-norm':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DAT_CROSS_ATTENTION_V1_Norm(width, heads, seq_len, depth, attn_mask, alpha, byta) for depth
                  in
                  range(layers)])
        elif dat_type == 'v1':
            print("v1")
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DAT_CROSS_ATTENTION(width, heads, seq_len, depth, attn_mask, alpha, byta) for depth
                  in
                  range(layers)])
        elif dat_type == 'v1-I':
            print("v1-I")
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DAT_CROSS_ATTENTION_IMAGE(width, heads, seq_len, depth, attn_mask, alpha, byta) for
                  depth
                  in
                  range(layers)])
        elif dat_type == 'v2':
            print("v2")
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DAT_CROSS_ATTENTION_V2(width, heads, seq_len, depth, attn_mask, alpha, byta) for
                  depth
                  in
                  range(layers)])
        elif dat_type == 'v3':
            print("v3")
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DAT_CROSS_ATTENTION_V3(width, heads, seq_len, depth, attn_mask, alpha, byta)
                  for
                  depth
                  in
                  range(layers)])
            ...
            print("base")
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_Base(width, heads, seq_len, depth, attn_mask, alpha, byta)
                  for
                  depth
                  in
                  range(layers)])
...
class VisionTransformer_DAT_PT(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int,
                 heads: int, output_dim: int, seq_len: int, dat_type: str, alpha=1, byta=1):
        super().__init__()
        self.h_resolution = h_resolution
        self.w_resolution = w_resolution
        self.output_dim = output_dim
        ...
        self.transformer = Transformer_DAT_PT(width, layers, heads, seq_len, dat_type=dat_type, alpha=alpha, byta=byta)
    ...
    def forward(self,
        ...
        x11 = self.transformer.resblocks[:-1]([x, use_dat, cv_emb])[0]
        x12 = self.transformer.resblocks[-1]([x11, use_dat, cv_emb])[0]
        x11 = x11.permute(1, 0, 2)  # LND -> NLD
        x12 = x12.permute(1, 0, 2)  # LND -> NLD
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int, seq_len=4, dat_type='dat', alpha=1, byta=1):
    ...
    print(state_dict["positional_embedding"].shape)
    ...
    model = CLIP(
        ...
        h_resolution, w_resolution, seq_len, dat_type, alpha, byta
    )
    if vit:
