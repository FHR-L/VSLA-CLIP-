...
class ResidualAttentionBlock_DAT_PT2(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None):
        ...
        self.alph = 4
        self.dat = nn.Sequential(OrderedDict([
    ...
    def forward(self, input):
        ...
        if cv_emb != None and self.deep < cv_emb.shape[0]: #[deep l (b t) d]
            cam_label = cv_emb[self.deep]
            l = cv_emb.shape[0]
            x = torch.cat([x, cam_label], dim=0)
        if use_dat:
            x = x + self.attention(self.ln_1(x))
            if cv_emb != None and self.deep < cv_emb.shape[0]:
                x = x[:-l]
...
class ResidualAttentionBlock_DAT_T(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None):
        super().__init__()
        ...
        #     ("up", nn.Linear(d_model // self.alph, d_model))
        # ]))
        self.dat_down = nn.Linear(d_model, d_model // self.alph)
        self.dat_attn = nn.MultiheadAttention(d_model // self.alph, n_head // self.alph)
        self.dat_attn2 = nn.MultiheadAttention(d_model // self.alph, n_head // self.alph)
        self.dat_up = nn.Linear(d_model // self.alph, d_model)
        self.dat_ln = LayerNorm(d_model // self.alph)
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
        self.deep = index
    ...
    def attention_dat(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.dat_attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]

    def attention_dat2(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.dat_attn2(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
    ...
    def forward(self, input):
        ...
        if use_dat:
            x = x + self.attention(self.ln_1(x))
            x_1 = self.dat_down(x)
            # x_1 = x_1 + self.attention_dat(x_1)
            x_1 = x_1 + self.attention_dat(x_1)
            # x_1 = x_1 + self.attention_dat2(x_1)
            x_1 = self.dat_up(x_1)
            x = x + self.mlp(self.ln_2(x)) + x_1
        else:
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
        return [x, use_dat, cv_emb]
...
class ResidualAttentionBlock_DAT_CROSSATTENTION(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None):
        super().__init__()
        ...
        self.dat = nn.Sequential(OrderedDict([
            ("down", nn.Linear(d_model, d_model // self.alph)),
            ("cross_frame_fusion", CrossAttentionBlock(d_model // self.alph, n_head // self.alph, attn_mask, 0, self.seqlen)),
            ("up", nn.Linear(d_model // self.alph, d_model))
        ]))
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
...
class ResidualAttentionBlock_DAT_CROSSATTENTION_V2(nn.Module):
    """
    这一版参考《AIM的设计方案，MLP的DAT不变，在multi-head attention后面增加一个dat，用来fusion多帧》
    """
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None):
        super().__init__()
        ...
        self.alph = 4
        self.dat_cross = nn.Sequential(OrderedDict([
            ("down", nn.Linear(d_model, d_model // self.alph)),
            ("cross_frame_fusion", CrossAttentionBlock(d_model // self.alph, n_head // self.alph, attn_mask, 0, self.seqlen)),
            ("up", nn.Linear(d_model // self.alph, d_model))
        ]))
        ...
        self.attn_mask = attn_mask
        self.deep = index
        self.dat_cross.apply(weights_init_kaiming)
        self.dat.apply(weights_init_kaiming)
...
class ResidualAttentionBlock_DAT_PT(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None):
        super().__init__()
...
class ResidualAttentionBlock_DAT_SIE_residual(nn.Module):
    def __init__(self, d_model: int, n_head: int, seq_len: int, index: int, attn_mask: torch.Tensor = None):
        super().__init__()
...
class Transformer_DAT_PT(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, seq_len: int, attn_mask: torch.Tensor = None, dat_type='dat'):
        ...
        if dat_type == 'dat':
            print("original dat")
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_DAT_PT(width, heads, seq_len, depth, attn_mask) for depth in range(layers)])
        elif dat_type == 'cross_v2':
            print("cross_v2")
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DAT_CROSSATTENTION_V2(width, heads, seq_len, depth, attn_mask) for depth in range(layers)])
        elif dat_type == 'cross_gelu':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DAT_CROSSATTENTION_V2(width, heads, seq_len, depth, attn_mask) for depth in
                  range(layers)])
