...
class PartDecoderLayer(nn.Module):
    def __init__(self, d_model=768, nhead=12, dim_feedforward=768*4, dropout=0.1, num_query=4, frame_num=8):
        super().__init__()
...
class PartDecoder(nn.Module):
    def __init__(self,  num_layers, num_query, pos_embed, frame_num=8):
        super().__init__()
        decoder_layer = PartDecoderLayer(num_query=num_query, frame_num=frame_num)
        width = 768
...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        ...
        if self.use_part_model:
            ...
            self.part_decoder = PartDecoder(num_layers=self.num_part_layer, num_query=self.num_part, pos_embed=self.image_encoder.positional_embedding, frame_num=cfg.DATALOADER.SEQ_LEN)
    ...
    def forward(self, x = None, label=None, get_image = False, get_text = False, cam_label= None, view_label=None):
        ...
        img_feature_proj = einops.rearrange(img_feature_proj, '(b t) d -> b t d', b=batch)
        img_feature_proj = torch.mean(img_feature_proj, dim=1)
        img_feature_map = einops.rearrange(img_feature_map, '(b t) l d -> b t l d', b=batch)
        if self.use_part_model:
            if self.use_part_token_as_query:
                part_features_query = self.part_decoder.forward_part_token_as_query(image_features[:, 1:])
            else:
                part_features_query = self.part_decoder.forward_cls_as_query(image_features[:, 1:],
                                                                             image_features[:, 0])
        ...
        if self.training:
            ...
            if self.use_part_model:
                ...
                # return torch.cat([part_feature_concat], dim=1)
            else:
