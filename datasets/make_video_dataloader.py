...
from datasets.video_loader import VideoDataset, VideoDatasetInfer, VideoDatasetWithImagePath
...
def make_dataloader(cfg):
    ...
    # queryloader_sampled_frames_a2g = DataLoader(
    #     VideoDataset(dataset.query_a2g, spatial_transform=spatial_transform_test,
    #                  temporal_transform=temporal_transform_test),
    #     batch_size=cfg.TEST.IMS_PER_BATCH, shuffle=False, num_workers=num_workers,
    #     pin_memory=True, drop_last=False)

    # galleryloader_sampled_frames_a2g = DataLoader(
    #     VideoDataset(dataset.gallery_a2g, spatial_transform=spatial_transform_test,
    #                  temporal_transform=temporal_transform_test),
    #     batch_size=cfg.TEST.IMS_PER_BATCH, shuffle=False, num_workers=num_workers,
    #     pin_memory=True, drop_last=False)
...
def make_dataloader_visualize(cfg):
    dataset = data_manager.init_dataset(name=cfg.DATASETS.NAMES, root=cfg.DATASETS.ROOT_DIR)
    num_workers = cfg.DATALOADER.NUM_WORKERS
    spatial_transform_train_stage2 = ST.Compose([
        ST.Scale(cfg.INPUT.SIZE_TRAIN, interpolation=3),
    ...
    queryloader_sampled_frames = DataLoader(
        VideoDatasetWithImagePath(dataset.query, spatial_transform=spatial_transform_test,
                     temporal_transform=temporal_transform_test),
        batch_size=cfg.TEST.IMS_PER_BATCH, shuffle=False, num_workers=num_workers,
        pin_memory=True, drop_last=False)

    galleryloader_sampled_frames = DataLoader(
        VideoDatasetWithImagePath(dataset.gallery, spatial_transform=spatial_transform_test,
                     temporal_transform=temporal_transform_test),
        batch_size=cfg.TEST.IMS_PER_BATCH, shuffle=False, num_workers=num_workers,
        pin_memory=True, drop_last=False)
