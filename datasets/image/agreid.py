...
class AG_ReID(BaseImageDataset):
    ...
    def _process_dir(self, dir_path, is_train=True):
        ...
        for img_path in img_paths:
            ...
            if is_train:
                if str(pid) in pid2label.keys():

                    pid = pid2label[str(pid)]
                    # pid_count[str(pid)] = pid_count[str(pid)] + 1
                else:
                    pid2label[str(pid)] = len(pid2label.keys())
                    pid = pid2label[str(pid)]
                    # pid_count[str(pid)] = 1
            data.append((img_path, pid, camid, 0))  # VSLA Don't need attributes
        # print(pid_count)
        # assert 0
        return data
